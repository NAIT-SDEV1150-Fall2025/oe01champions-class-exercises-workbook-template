...
Browser-native APIS are built-in features provided by web browsers that developer can use without installing any external libraries.
DOM-API: document.querySelector('id')
Fetch API -  to get the data from client and post to the server/database or vice versa.


A web component is a set of browser native APIS ( Web Components API) that have built in methods/features/classes for developers to create custom html elements.

Web components API

Web component using Web component API have four main walls/technologies.
